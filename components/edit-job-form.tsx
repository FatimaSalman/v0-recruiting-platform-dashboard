...
export function EditJobForm({ jobId }: EditJobFormProps) {
  ...
  async function fetchJob() {
    try {
      ...
      setError(t("jobs.form.errorLoading"))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from("jobs").update(jobData).eq("id", jobId)

      if (updateError) throw updateError

      router.push("/dashboard/jobs")
    } catch (err: any) {
      setError(err.message || t("jobs.form.errorUpdating"))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold tracking-tight mb-2">{t("jobs.form.editTitle")}</h1>
        <p className="text-muted-foreground">{t("jobs.form.editSubtitle")}</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <Button type="submit" disabled={saving}>
            {saving ? t("jobs.form.updating") : t("jobs.form.updateJob")}
          </Button>
        </div>
      </form>
    </div>
